...
Q-learning을 업데이트 할 때 ![eq2](image/eq22.png) 손실 함수를 사용합니다. 이 때 gamma는 할인율로 agent의 미래 horizon을 결정하고, theta_i는 현재 반복 단계 i에서의 Q-Network 파라미터이고, theta^-_i는 같은 반복 단계에서 target Network를 위한 파라미터입니다.
