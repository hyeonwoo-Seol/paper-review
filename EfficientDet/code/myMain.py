...
class BiFPNLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # --- Bottom-Up Path ---
        # ❗수정: self.swish() 호출 제거
        w5 = self.w5_relu(self.w5); w5 = w5 / (torch.sum(w5, dim=0) + self.epsilon)
            ...
            with open(label_path, 'r') as f:
                for line in f.readlines():
                    ...
                    if class_name in self.class_to_idx:
                        # KITTI 라벨 형식: class, truncated, occluded, alpha x1,
                        x1 = float(parts[4])
                        y1 = float(parts[5])
                        x2 = float(parts[6])
                        y2 = float(parts[7])
...
# DataLoader는 배치 내의 샘플 크기가 다를 때 이를 처리하기 위해 collate_fn을 사용한다.
...
if __name__ == "__main__":
    ...
    # 데이터 준비
    # 실제로는 이미지와 바운딩 박스를 함께 변환하는 albumentations 라이브러리 사용을 권장한다.
